...
export const getCourseByIdController = asyncHandler(async (req, res) => {
  ...
  res
    ...
    .json(successResponse(courseData, "Course retrieved successfully"));
});
...
export const createCourseController = asyncHandler(async (req, res) => {
  ...
  const titleExists = await courseTitleExistsForInstructor(
    title,
    instructor_id
  );
  if (titleExists) {
  ...
  const courseData = {
    ...
    category_id,
    instructor_id,
    price: 0.0, // All courses are free as requested
    level,
    duration_weeks,
    is_published,
  };
...
export const updateCourseController = asyncHandler(async (req, res) => {
  ...
  res
    .status(HTTP_STATUS.OK)
    .json(successResponse(updatedCourse, "Course updated successfully"));
});
...
export const getInstructorCourses = asyncHandler(async (req, res) => {
  ...
  res
    .status(HTTP_STATUS.OK)
    .json(
      successResponse(courses, "Instructor courses retrieved successfully")
    );
});
...
export const getFeaturedCoursesController = asyncHandler(async (req, res) => {
  ...
  res
    .status(HTTP_STATUS.OK)
    .json(successResponse(courses, "Featured courses retrieved successfully"));
});
...
export const getCourseStatsController = asyncHandler(async (req, res) => {
  ...
  if (user.role !== USER_ROLES.ADMIN && course.instructor_id !== user.id) {
    throw new AppError(
      "Access denied. You can only view stats for your own courses.",
      HTTP_STATUS.FORBIDDEN
    );
  }
  ...
  res
    .status(HTTP_STATUS.OK)
    .json(successResponse(stats, "Course statistics retrieved successfully"));
});
...
export const toggleCoursePublishStatus = asyncHandler(async (req, res) => {
  ...
  if (user.role !== USER_ROLES.ADMIN && course.instructor_id !== user.id) {
    throw new AppError(
      "Access denied. You can only publish/unpublish your own courses.",
      HTTP_STATUS.FORBIDDEN
    );
  }
  ...
  res
    .status(HTTP_STATUS.OK)
    .json(successResponse(updatedCourse, `Course ${action} successfully`));
});
...
export const getCoursesByCategoryController = asyncHandler(async (req, res) => {
  ...
  res
    .status(HTTP_STATUS.OK)
    .json(successResponse(courses, "Courses retrieved successfully"));
});
...
export const duplicateCourse = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    originalCourse.instructor_id !== user.id
  ) {
    throw new AppError(
      "Access denied. You can only duplicate your own courses.",
      HTTP_STATUS.FORBIDDEN
    );
  }
  ...
  res
    .status(HTTP_STATUS.CREATED)
    .json(successResponse(newCourse, "Course duplicated successfully"));
});
